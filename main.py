...
from PySide6.QtWidgets import (
    ...
    QRadioButton, QVBoxLayout, QHBoxLayout, QFileDialog
)
...
def generate_bounding_box_map_html(bounds: list):
    html = f"""
        ...
        <script>
            var map = L.map('map').setView([10.77545, 106.6599], 15);
...
class Application(QMainWindow):
    ...
    def run_shortest_path_algorithm(self):
        if self.handler is not None:
            children = self.chart_web_layout.children()
            for child in children:
                self.chart_web_layout.removeWidget(child)
    ...
    def load_osm_file(self):
        ...
        if os.path.isfile(filepath[0]):
            self.handler = pmd.OSMProcessHandler.read_osm_file(filepath[0])
            chart = QChart()
            chart.setTitle("Simple Line Chart")
            chart.createDefaultAxes()
            self.view_left = QChartView(chart)
            self.view_right = WebEngineView(
                html_content=generate_bounding_box_map_html(self.handler.get_bounds()))
